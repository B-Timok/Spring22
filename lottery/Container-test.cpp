...
TEMPLATE_TEST_CASE("Container(Container&&)", "", char, int, double) {
    const Container<TestType> REF { 65, 66, 67, 68, 69, 70, 71, 72 };
    Container<TestType> box1(std::move(Container<TestType>{ 65, 66, 67, 68, 69, 70, 71, 72 }));
...
TEMPLATE_TEST_CASE("Container::at()", "", char, int, double) {
    const Container<TestType> REF1 { 65, 66, 67, 68, 69, 70, 71, 72 };
    Container<TestType> ref2(REF1);

    for (size_t index = 0; index < REF1.size(); ++index) {
        CHECK(REF1.at(index) == ref2.at(index));
        ref2.at(index) += 32;
        CHECK(ref2.at(index) == REF1.at(index) + 32);
    }

    CHECK_THROWS_AS(REF1.at(-1), std::out_of_range);
    CHECK_THROWS_AS(ref2.at(ref2.size()) = 42, std::out_of_range);
}

TEMPLATE_TEST_CASE("Container::operator[]()", "", char, int, double) {
    const Container<TestType> REF1 { 65, 66, 67, 68, 69, 70, 71, 72 };
    Container<TestType> ref2(REF1);

    for (size_t index = 0; index < REF1.size(); ++index) {
        CHECK(REF1[index] == ref2[index]);
        ref2[index] += 32;
        CHECK(ref2[index] == REF1[index] + 32);
    }

    CHECK_NOTHROW(REF1[-1] == 0);
    CHECK_NOTHROW(ref2[ref2.size()] = 0);
}
...
TEMPLATE_TEST_CASE("Container& operator=(Container&&)", "", char, int, double) {
    ...
    Container<TestType> box1{};
    box1 = std::move(Container<TestType>(REF));
...
TEMPLATE_TEST_CASE("swap(Container&)", "", char, int, double) {
    ...
    CHECK(std::equal(box1.begin(), box2.begin(), REF.begin(), REF.end()) == false);
